...
class EstateService extends GetxService {
  ...
  Future<void> fetchEstatesRaw() async {
    try {
      ...
      print(
          '🔥 FETCH ESTATES: Making direct HTTP call to bypass typing issues');
      print('🔥 FETCH ESTATES RAW: Starting direct HTTP call');

      final response = await http.get(
        Uri.parse('${Environment.apiBaseUrl}estates'),
        headers: {
      ...
      print('🌐 HTTP GET Request: ${Environment.apiBaseUrl}estates');
      print(
          '🔥 FETCH ESTATES RAW: Response status code: ${response.statusCode}');

      if (response.statusCode == 200) {
        final Map<String, dynamic> jsonData = json.decode(response.body);
        print(
            '🔥 FETCH ESTATES RAW: Response data type: ${jsonData.runtimeType}');
        ...
        if (jsonData['success'] == true && jsonData['data'] != null) {
          ...
          estates.value =
              estateData.map((json) => Estate.fromJson(json)).toList();

          if (estates.isNotEmpty) {
            print(
                '🔥 FETCH ESTATES: First estate raw data: ${estateData.first}');
            print(
                '🔥 FETCH ESTATES: First estate parsed - Name: ${estates.first.name}, City: ${estates.first.address.city}');
          }
          print(
              '🔥 FETCH ESTATES: Successfully loaded ${estates.length} estates');
        } else {
          print(
              '🔥 FETCH ESTATES ERROR: API returned success=false or no data');
          Get.snackbar('Error', jsonData['error'] ?? 'Failed to load estates');
        }
      } else {
        print('🔥 FETCH ESTATES ERROR: HTTP ${response.statusCode}');
        Get.snackbar(
            'Error', 'Failed to load estates: HTTP ${response.statusCode}');
      }
  ...
  Future<void> fetchUnitsRaw({String? estateId}) async {
    try {
      isLoading.value = true;
      print(
          '🔥 FETCH UNITS: Starting to fetch units for estate: ${estateId ?? "all"}');
      ...
      String url = '${Environment.apiBaseUrl}units';
      ...
      if (response.statusCode == 200) {
        final Map<String, dynamic> jsonData = json.decode(response.body);
        print(
            '🔥 FETCH UNITS RAW: Response data type: ${jsonData.runtimeType}');
        ...
        if (jsonData['success'] == true && jsonData['data'] != null) {
          ...
          for (int i = 0; i < unitData.length; i++) {
            try {
              ...
              print(
                  '🔥 FETCH UNITS: Successfully processed unit $i - ${unit.unitNumber}');
            } catch (e) {
          ...
          print(
              '🔥 FETCH UNITS: Successfully loaded ${units.length} out of ${unitData.length} units');
        } else {
        ...
        Get.snackbar(
            'Error', 'Failed to load units: HTTP ${response.statusCode}');
      }
  ...
  Future<Estate?> getEstateById(String estateId) async {
    try {
      ...
      final response = await http.get(
        Uri.parse('${Environment.apiBaseUrl}estates/$estateId'),
        headers: {
          'Content-Type': 'application/json',
          'Authorization': 'Bearer $token',
        },
      ...
      print(
          '🔥 GET ESTATE BY ID: Response status code: ${response.statusCode}');
      if (response.statusCode == 200) {
        final Map<String, dynamic> jsonData = json.decode(response.body);
        if (jsonData['success'] == true && jsonData['data'] != null) {
  ...
  Future<Unit?> getUnitById(String unitId) async {
    try {
      ...
      final response = await http.get(
        Uri.parse('${Environment.apiBaseUrl}units/$unitId'),
        headers: {
          'Content-Type': 'application/json',
          'Authorization': 'Bearer $token',
        },
  ...
  List<Estate> searchEstates(String query) {
    ...
    return estates
        .where((estate) =>
            ...
            estate.address.city?.toLowerCase().contains(query.toLowerCase()) ==
                true ||
            estate.address.province
                    ?.toLowerCase()
  ...
  List<Unit> getUnitsByEstate(String estateId) {
    return units.where((unit) => unit.estateId == estateId).toList();
  }
  ...
  Future<void> fetchEstatesWithUnits() async {
    try {
      isLoading.value = true;
      print(
          '🔥 FETCH ESTATES WITH UNITS: Starting to fetch estates that have units...');
      ...
      final response = await http.get(
        Uri.parse('${Environment.apiBaseUrl}estates?hasUnits=true'),
        headers: {
          'Content-Type': 'application/json',
          'Authorization': 'Bearer $token',
        },
      ...
      print(
          '🌐 HTTP GET Request: ${Environment.apiBaseUrl}estates?hasUnits=true');
      print(
          '🔥 FETCH ESTATES WITH UNITS: Response status code: ${response.statusCode}');

      if (response.statusCode == 200) {
        final Map<String, dynamic> jsonData = json.decode(response.body);
        print(
            '🔥 FETCH ESTATES WITH UNITS: Response success: ${jsonData['success']}');

        if (jsonData['success'] == true && jsonData['data'] != null) {
          final List<dynamic> estateData = jsonData['data'] as List<dynamic>;
          print(
              '🔥 FETCH ESTATES WITH UNITS: Processing ${estateData.length} estates');

          estates.value =
              estateData.map((json) => Estate.fromJson(json)).toList();
          print(
              '🔥 FETCH ESTATES WITH UNITS: Successfully loaded ${estates.length} estates with units');
        } else {
          print(
              '🔥 FETCH ESTATES WITH UNITS ERROR: API returned success=false or no data');
          Get.snackbar('Error', jsonData['error'] ?? 'Failed to load estates');
        }
      } else {
        ...
        print(
            '🔥 FETCH ESTATES WITH UNITS: Falling back to manual filtering...');
  ...
  Future<void> fetchEstatesAndFilterByUnits() async {
    try {
      print(
          '🔥 FILTERING ESTATES: Fetching all estates and filtering manually...');
      ...
      for (Estate estate in estates) {
        print(
            '🔥 FILTERING ESTATES: Checking units for ${estate.name} (${estate.id})');
        ...
        if (units.isNotEmpty) {
          print(
              '🔥 FILTERING ESTATES: ${estate.name} has ${units.length} units - INCLUDED');
      ...
      print(
          '🔥 FILTERING ESTATES: Final result - ${estates.length} estates with units');
    } catch (e) {
