...
import { KhanyiAuthService } from '../../core/services/khanyi-auth.service';
...
export class SidebarComponent {
  ...
  ngOnInit(): void {
    // Subscribe to authentication state changes to update menu
    this.authService.currentUser$.subscribe(user => {
      console.log('🔄 Sidebar: User changed, updating menu for role:', user?.role || 'none');
  ...
  private filterMenuByRole(menuItems: MenuItem[]): MenuItem[] {
    ...
    return menuItems.map(item => {
      // For title items (section headers), keep them if they have visible children
      if (item.isTitle) {
        return item;
      }
      ...
      if (item.subItems && item.subItems.length > 0) {
        const filteredSubItems = item.subItems.filter((subItem: MenuItem) => {
